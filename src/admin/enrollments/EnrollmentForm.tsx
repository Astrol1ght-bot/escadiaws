...
import { CreateEnrollmentInput, GetEnrollmentQuery, EnrollmentStatus } from '../../API';
...
export const EnrollmentForm: React.FC<Props> = ({ isLoading, onSubmit, title, order }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    <Form
      actions={
        <SpaceBetween direction='horizontal' size='xs'>
          <Button onClick={() => navigate('/admin/enrollments')}>Cancelar</Button>
          <Button
            loading={isLoading}
            formAction='submit'
      ...
      header={<Header description=''>{title}</Header>}
    >
        ...
        <ul className='list-none p-0 m-0'>
          <li className='flex align-items-center py-3 px-2 border-top-1 border-300 flex-wrap'>
            ...
            <div className='text-900 w-full md:w-8 md:flex-order-0 flex-order-1'>
              {order?.getEnrollment?.student?.name}
            </div>
          </li>
          ...
          <li className='flex align-items-center py-3 px-2 border-top-1 border-300 flex-wrap'>
            ...
            <div className='text-900 w-full md:w-8 md:flex-order-0 flex-order-1'>
              {order?.getEnrollment?.student?.emai}
            </div>
          </li>
          ...
          <li className='flex align-items-center py-3 px-2 border-top-1 border-300 flex-wrap'>
            ...
            <div className='text-900 w-full md:w-8 md:flex-order-0 flex-order-1'>
              {order?.getEnrollment?.courseName}
            </div>
          </li>
